...
const App: React.FC = () => {
  ...
  // Debug: Log fheUtils status
  useEffect(() => {
    console.log("🔍 App: fheUtils status", {
      connected,
      fheUtils: !!fheUtils,
      account: !!account,
      enabled: connected && !!fheUtils,
    });
  }, [connected, fheUtils, account]);
    ...
    try {
      ...
      Promise.all([
        ...
        new Promise((resolve) => {
          const checkSDK = () => {
            if (isReady) {
              console.log("🟩 connectWallet: SDK ready → reloadUserState");
      ...
      console.log("🟩 connectWallet: set state done", { address });
    } catch (error: any) {
  ...
  useEffect(() => {
    if (sdk && isReady && provider && signer) {
      try {
        console.log("🔧 App: Initializing fheUtils...");
        initializeFheUtils(sdk, provider, signer);
        console.log("✅ App: fheUtils initialized");
      } catch (e) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.log("🔐 App: Requesting user-decrypt authorization...");
        const ok = await (fheUtils as any).requestUserDecryptAuthorization();
        if (ok) {
          console.log("✅ App: User-decrypt authorization granted");
          try {
          ...
          console.log("❌ App: User-decrypt authorization failed");
        }
      } catch (e) {
  ...
  const refreshUserData = useCallback(async () => {
    try {
      console.log("🔄 refreshUserData: calling reloadUserState()");
      await reloadUserState();
      console.log("✅ refreshUserData: done");
    } catch {}
  }, [reloadUserState]);
  ...
  useEffect(() => {
    ...
    console.log("🔄 App: Starting initial data load...", { connected, isReady, account });

    // Add error handling for the initial load
    const loadData = async () => {
      try {
        await refreshUserData();
        console.log("✅ App: Initial data load completed");
      } catch (error) {
  ...
  const handleBuyGmTokens = useCallback(async () => {
    try {
      ...
      setTxStatus("pending");
      // Enforce FHE-only path with retries; no ETH value sent
      const hasBuyGmFhe = typeof (fheUtils as any).contract.buyGmTokensFHE === "function";
      if (!hasBuyGmFhe) throw new Error("buyGmTokensFHE not available");

      const maxAttempts = 2; // reduce retries to improve perceived speed
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
          ...
          console.log("🟨 handleBuyGmTokens: sending tx buyGmTokensFHE");
          const tx = await (fheUtils as any).contract.buyGmTokensFHE(handles[0], inputProof, {
            gasLimit: 900_000,
            maxPriorityFeePerGas: priority,
            maxFeePerGas: maxFee,
          });
          console.log("⏳ handleBuyGmTokens: tx submitted", tx?.hash);
          await tx.wait();
          console.log("✅ handleBuyGmTokens: tx confirmed");
          setTxStatus("success");
      ...
      // Fallback: use public ETH path (still credits encrypted GM on-chain) when relayer is down
      try {
        ...
        await tx2.wait();
        setTxStatus("success");
        update(toastId, "success", "Purchased GM with ETH (fallback)", 2500);
        setTimeout(() => {
          try {
            (reloadUserState as any)?.(true, true);
          } catch {}
        ...
        console.error("🟥 handleBuyGmTokens: fallback failed", (fallbackErr as any)?.message);
        update(toastId, "error", "Relayer down and ETH fallback failed", 4000);
        throw new Error(`Relayer unavailable: ${lastErr?.message || lastErr}`);
      }
    } catch (e: any) {
      console.error("🟥 handleBuyGmTokens: error", e);
      setTxStatus("error");
  ...
  const handleDailyGm = useCallback(async () => {
    try {
      console.log("🟦 handleDailyGm: start");
      requireReady();
      ...
      try {
        ...
        console.log("🟨 handleDailyGm: estimate gas", est.toString());
      } catch {}
      ...
      const base = fee.maxFeePerGas || 20n * 10n ** 9n;
      const maxFee = base + priority;
      console.log("🟨 handleDailyGm: sending tx dailyGm");
      const tx = await (fheUtils as any).contract.dailyGm({
        gasLimit,
        maxPriorityFeePerGas: priority,
        maxFeePerGas: maxFee,
      });
      console.log("⏳ handleDailyGm: tx submitted", tx?.hash);
      const receipt = await tx.wait();
      console.log("✅ handleDailyGm: tx confirmed");
      try {
        ...
        if (evt) {
          ...
          console.log("🟩 handleDailyGm: event CheckInCompleted", ts);
        }
      } catch {}
      setTxStatus("success");
  ...
  const handleSpin = useCallback(async () => {
    try {
      console.log("🟦 handleSpin: start", { userDataLoading, availableSpins });
      requireReady();
      if (userDataLoading) {
        console.warn("🟨 handleSpin: blocked because userDataLoading");
        push("error", "Loading private data...", 2500);
        return;
      }
      if (!Number.isFinite(availableSpins) || availableSpins <= 0) {
        console.warn("🟨 handleSpin: blocked because no spins");
      ...
      // Do not pre-gate by decrypted/event spins; rely on on-chain FHE gate
      const toastId = push("pending", "Submitting spin transaction...");
      setTxStatus("pending");
      console.log("🟨 handleSpin: calling fheUtils.spin()");
      const resultStr = await fheUtils!.spin();
      console.log("🟩 handleSpin: spin returned", resultStr);
  ...
  const handleClaimETH = useCallback(async () => {
    try {
      ...
      // Backend attestation: call /api/claim-attestation to get signature
      let response = await fetch(`${CONFIG.BACKEND_API_URL}/claim-attestation`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          nonce: (await (fheUtils as any).contract.claimNonce(account)).toString(),
        }),
      });

      // Fallback: if proxy '/api' not wired (404), try direct localhost:4009 in dev
      if (!response.ok && response.status === 404 && /localhost|127\.0\.0\.1/.test(window.location.hostname)) {
        try {
          response = await fetch(`http://localhost:4009/claim-attestation`, {
            method: "POST",
      ...
      if (!response.ok) throw new Error("Failed to get attestation from oracle");

      const { signature } = await response.json();

      // Call contract with attestation signature
      const tx = await (fheUtils as any).contract.claimETH(amountWei, signature, {
        gasLimit: 500_000, // Tăng gas limit để đảm bảo đủ gas
        maxFeePerGas: ethers.parseUnits("50", "gwei"), // EIP-1559 gas price
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button
              ...
              onClick={() => {
                push("pending", "Waiting 15 seconds for cooldown...", 15000);
                setTimeout(() => {
                  push("success", "Cooldown finished, retrying...", 2000);
                  reloadUserState(false, true);
                }, 15000);
              }}
        ...
        <div className="spin-wheel-container">
          ...
          <SpinWheel
            ...
            onSpinComplete={(result) => {
              ...
              // After spin animation ends: settle prize, then reload data (MetaMask signs now)
              (async () => {
                try {
                  if (slotForSettlement >= 0 && slotForSettlement <= 7) {
                    const fee2 = await provider!.getFeeData();
                    const pr2 = ((fee2.maxPriorityFeePerGas || 2n * 10n ** 9n) * 13n) / 10n;
                    const bf2 = fee2.maxFeePerGas || 20n * 10n ** 9n;
                    const mf2 = bf2 + pr2;
                    const tx2 = await (fheUtils as any).contract.settlePrize(slotForSettlement, {
                      gasLimit: 800_000, // Tăng gas limit cho settlePrize
                      maxPriorityFeePerGas: pr2,
                      maxFeePerGas: mf2,
                    });
                    // SỬA: Chỉ reload 1 lần sau khi settlePrize confirm
                    if (tx2) {
                      await tx2.wait();
                  ...
                  // XÓA: Không reload thêm lần nữa ở đây
                } catch {}
              })();
              // SỬA: Chỉ reload leaderboard, không reload user data nữa
              setTimeout(() => {
                try {
                  loadLeaderboard();
                } catch {}
          ...
          <div className="spin-section">
            ...
            <button
              ...
              disabled={!connected || isBuyingSpins}
            >
              🔥 Buy Spins (GM)
            </button>
          </div>
      ...
      <div className="sidebar">
        <div className="card">
          ...
          <button
            ...
            title="Claim ETH with oracle attestation"
