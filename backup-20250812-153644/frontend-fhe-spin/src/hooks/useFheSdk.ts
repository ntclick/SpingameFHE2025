...
const loadSDK = async (): Promise<any> => {
  return new Promise((resolve, reject) => {
    if (typeof window !== "undefined") {
      ...
      if (globalSdk) {
        console.log("✅ SDK loaded from global:", typeof globalSdk);
        resolve(globalSdk);
        return;
      }

      // Wait for SDK to load if not immediately available
      let attempts = 0;
      ...
      const checkSDK = () => {
        ...
        if (sdk) {
          console.log("✅ SDK loaded after waiting:", typeof sdk);
          resolve(sdk);
          return;
        }
        if (attempts >= maxAttempts) {
          reject(new Error("SDK failed to load after 5 seconds"));
          return;
        }
...
const initializeSDK = async (provider: ethers.BrowserProvider, signer: ethers.Signer): Promise<any> => {
  try {
    ...
    // Debug: Log relayer URL
    console.log("🔍 SDK Config:", {
      relayerUrl: instanceConfig.relayerUrl,
      isLocalhost: typeof window !== "undefined" && /localhost|127\.0\.0\.1/.test(window.location.hostname),
      envRelayerUrl: process.env.REACT_APP_RELAYER_URL,
    });
  ...
  const createEncryptedInput = useCallback(
    async (
      ...
      try {
        ...
        console.log("🔐 Creating encrypted input:", { contractAddress, account, plainValues });
        ...
        if (typeof state.sdk.createEncryptedInput === "function") {
          try {
            ...
            console.log("✅ Encrypted input created with SDK:", { handles, inputProof });
        ...
        console.log("⚠️ Using fallback encrypted input implementation");
        ...
        try {
          ...
          for (let i = 0; i < plainValues.length; i++) {
            ...
            try {
              ...
              if (state.sdk.encrypt && typeof state.sdk.encrypt === "function") {
                ...
                console.log(`✅ Encrypted value ${value} using SDK encrypt:`, encrypted);
              }
              // Try to use SDK's encrypt64 method
                ...
                encryptedArguments.push(encrypted);
                console.log(`✅ Encrypted value ${value} using SDK encrypt64:`, encrypted);
              }
              // ✅ Fallback to bytes32 format for externalEuint64
                ...
                console.log(`✅ Encrypted value ${value} using bytes32 format:`, encrypted);
              }
            } catch (encryptError) {
          ...
          console.log("✅ Fallback encrypted input created:", {
    ...
    async (ciphertext: any): Promise<FheOperationResult<number>> => {
      try {
        ...
        try {
          ...
          console.log(`✅ Real FHE Decrypt: ${ciphertext.slice(0, 20)}... -> ${numericValue}`);
    ...
    (): AclOperation => ({
      grantAccess: async (user: string, data: any): Promise<boolean> => {
        try {
          ...
          console.log(`🔐 ACL: Granting access to ${user} for data`);
          return true;
        } catch (error) {
      ...
      checkAccess: async (user: string, data: any): Promise<boolean> => {
        try {
          ...
          console.log(`🔍 ACL: Checking access for ${user}`);
          return true;
        } catch (error) {
      ...
      revokeAccess: async (user: string, data: any): Promise<boolean> => {
        try {
          ...
          // In a real implementation, this would call contract methods
          console.log(`🚫 ACL: Revoking access for ${user}`);
          return true;
        } catch (error) {
