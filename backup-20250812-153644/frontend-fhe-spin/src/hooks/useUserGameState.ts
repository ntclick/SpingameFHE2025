...
export default function useUserGameState(account: string | null | undefined, enabled: boolean) {
  ...
  // Throttling/TTL controls - TĂNG LÊN ĐỂ TRÁNH SPAM
  const MIN_RELOAD_INTERVAL_MS = 10000; // Tăng từ 3000ms lên 10000ms
  const CACHE_TTL_MS = 30_000; // Tăng từ 15s lên 30s
  ...
  const fetchBundle = useCallback(async (): Promise<UserGameState | null> => {
    if (!enabled || !account || !fheUtils) {
      console.log("❌ fetchBundle: Missing requirements", { enabled, account: !!account, fheUtils: !!fheUtils });
      return null;
    }
    console.log("🔄 fetchBundle: Starting...", { account });
    try {
      const c: any = fheUtils.contract;

      // Debug: Kiểm tra contract
      console.log("🔍 fetchBundle: Contract info", {
        contractAddress: c?.target,
        hasStateVersion: typeof c?.stateVersion === "function",
        hasGetUserSpins: typeof c?.getUserSpins === "function",
        hasGetUserGmBalance: typeof c?.getUserGmBalance === "function",
        hasGetEncryptedPendingEthWei: typeof c?.getEncryptedPendingEthWei === "function",
        hasGetUserScore: typeof c?.getUserScore === "function",
      });
      ...
      // Debug: Kiểm tra encrypted data
      console.log("🔍 fetchBundle: Encrypted data", {
        version,
        spinsEnc: spinsEnc?.slice(0, 20) + "...",
        gmEnc: gmEnc?.slice(0, 20) + "...",
        pendingEnc: pendingEnc?.slice(0, 20) + "...",
        scoreEnc: scoreEnc?.slice(0, 20) + "...",
      });
      ...
      console.log("✅ fetchBundle: Success", result);
      return result;
    } catch (e: any) {
    ...
    (async () => {
      ...
      try {
        ...
        if (bundle && !cancelled) {
          ...
          setData(bundle);
          setUsingFallback(usingFallbackRef.current);
          console.log("✅ Loaded from contract:", bundle);
        }
      } catch (e: any) {
        if (!cancelled) {
    ...
    const handler = (user: string, versionBn: any) => {
      try {
        ...
        if (v !== currentVersionRef.current) {
          // TỐI ƯU: Reload ngay lập tức không debounce
          reload(false, true);
        }
      } catch {}
    };
