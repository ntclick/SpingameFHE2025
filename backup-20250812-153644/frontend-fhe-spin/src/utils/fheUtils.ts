...
// ✅ LuckySpinFHE_Strict ABI - FHEVM compatible (strict FHE)
const LuckySpinFHE_abi = [
  {
    inputs: [],
  ...
  // (constants are not strictly needed in ABI for our usage)
  {
    inputs: [
...
export class FheUtils {
  ...
  static setDecryptCooldown(value: boolean): void {
    FheUtils.decryptCooldown = value ? 200 : 0; // Giảm cooldown để load nhanh hơn
  }
  ...
  async decryptEuint64(ciphertext: string): Promise<bigint> {
    ...
    try {
      ...
      console.log("decryptEuint64 cached auth failed:", e?.message);
    }
    ...
    try {
      ...
      console.log("decryptEuint64 new auth failed:", e?.message);
    }
  ...
  async decryptMultipleValues(
    ...
    if (!cachedAuth) {
      console.log("❌ decryptMultipleValues: No cached auth");
      return {};
    }
    ...
    if (FheUtils.decryptCooldown > 0) {
      console.log("⏳ decryptMultipleValues: In cooldown, skipping");
      return {};
    }

    const decryptPromise = (async () => {
      try {
        console.log("🔄 decryptMultipleValues: Starting batch decrypt", { count: handleContractPairs.length });
        ...
        console.log("✅ decryptMultipleValues: Success", {
          ...
          values: Object.fromEntries(
            Object.entries(bigintResult).map(([k, v]) => [k.slice(0, 10) + "...", v.toString()]),
          ),
        });
  ...
  async buySpins(amount: number, ethValue: string) {
    try {
      console.log("💰 Buying spins with FHE:", { amount, ethValue });
      ...
      console.log("⏳ Buy spins transaction sent:", tx.hash);
      const receipt = await tx.wait();
      console.log("✅ Buy spins completed:", receipt.hash);
  ...
  // ✅ Spin với FHE và decrypt result
  async spin(): Promise<string> {
    try {
      console.log("🎰 Spinning with FHE...");

      // Estimate gas with buffer; fallback to generous cap if estimate fails
      let overrides: any = {};
      try {
        const est: bigint = await (this.contract as any).estimateGas.spin();
        const buffered = (est * 20n) / 10n; // 2.0x safety
      ...
      // Add EIP-1559 fee overrides to improve inclusion on Sepolia
      try {
        const feeData = await (this.signer.provider as any)?.getFeeData?.();
        const maxFee = feeData?.maxFeePerGas ?? ethers.parseUnits("3", "gwei");
        const maxPrio = feeData?.maxPriorityFeePerGas ?? ethers.parseUnits("2", "gwei");
        overrides.maxFeePerGas = maxFee;
        overrides.maxPriorityFeePerGas = maxPrio;
      } catch {}

      // Call contract method directly to ensure calldata is set
      // Use spinLite to minimize HCU and rely on settlePrize later
      const tx = await (this.contract as any).spinLite(overrides);
      console.log("⏳ Spin transaction sent:", tx.hash);

      // Race between event listener and receipt parsing to reduce perceived latency
      const user = await this.signer.getAddress();

      const waitForEvent = new Promise<string>((resolve) => {
        const handler = (evtUser: string, slot: number, prizeWei: any, gmDelta: any) => {
          if (evtUser.toLowerCase() === user.toLowerCase()) {
            try {
              this.contract.off("SpinOutcome", handler as any);
            } catch {}
            resolve(JSON.stringify({ slotIndex: Number(slot), gmDelta: Number(gmDelta) }));
          }
        };
        this.contract.on("SpinOutcome", handler as any);
        // Safety timeout to remove listener later if needed
        setTimeout(() => {
          try {
            this.contract.off("SpinOutcome", handler as any);
          } catch {}
      ...
      const waitForReceipt: Promise<string> = (async (): Promise<string> => {
        const receipt = await tx.wait();
        console.log("✅ Spin mined:", receipt.transactionHash);
      ...
      const resultStr: string = (await Promise.race([waitForEvent, waitForReceipt])) || "";
      console.log("🎯 Spin result from event:", { result: resultStr });
      return resultStr;
    } catch (error) {
  ...
  async claimPendingEth(desiredEth?: string): Promise<void> {
    ...
    // Strategy A: if current signer is attestor, self-sign
    const attestor: string = await (this.contract as any).attestor();
    const caller = (await this.signer.getAddress()).toLowerCase();
    const nonce: bigint = BigInt(await (this.contract as any).claimNonce(user));
    let signature: string | null = null;
    if (attestor?.toLowerCase?.() === caller) {
      signature = await signClaimAttestation(this.signer as any, this.contract.target as string, user, claimWei, nonce);
    } else {
      // Strategy B: request signature from external attestor backend if configured
      try {
        const api = (CONFIG as any).ATTESTOR_API_URL as string;
        if (api) {
          const res = await fetch(`${api}/claim-sign`, {
            ...
            body: JSON.stringify({
              ...
              user,
              wei: claimWei.toString(),
              nonce: nonce.toString(),
            }),
          });
          if (res.ok) {
            const json = await res.json();
            if (json?.signature) signature = json.signature;
          }
        }
      } catch {}
      if (!signature) throw new Error("Claim requires oracle attestation. Please use backend to request claim.");
    }
    const tx = await (this.contract as any).claimETH(claimWei, signature);
    await tx.wait();
  }
