...
/// @title LuckySpinFHE_Optimized - Contract tối ưu theo FHEVM Encrypted Inputs Standards
/// @author fhevm-hardhat-template
/// @notice Contract với input packing tối ưu theo tài liệu FHEVM
/// @dev Sử dụng single proof cho multiple encrypted inputs
contract LuckySpinFHE_Optimized is SepoliaConfig {
    ...
    event OptimizedInputProcessed(address indexed user, uint256 inputCount);

    // ===== 6. OPTIMIZED INPUT VALIDATION =====

    /// @notice Validate input proof theo tài liệu FHEVM
    ...
    function validateInputProof(bytes calldata inputProof) internal view returns (bool) {
        ...
        // Additional validation logic có thể thêm ở đây
        return true;
    }

    /// @notice Validate encrypted inputs với single proof
    /// @param encryptedSpins encrypted spins
    ...
    function validateEncryptedInputs(
        ...
        // FHE.fromExternal() sẽ tự động validate khi được gọi
        return true;
    }
    ...
    // ===== 8. OPTIMIZED SPIN LOGIC (Single Proof) =====

    /// @notice Optimized spin với single proof (theo tài liệu FHEVM)
    ...
    function optimizedSpinAndClaimReward(
        ...
        // Validate encrypted inputs với single proof
        require(
            validateEncryptedInputs(encryptedSpins, encryptedPoolIndex, encryptedPoint, inputProof),
        ...
        // Convert external data với single proof (tối ưu theo tài liệu)
        ...
        // Enhanced spin logic với random generation
        ...
        // Enhanced spin consumption logic với random factor
        ...
        emit OptimizedInputProcessed(msg.sender, 3); // 3 inputs processed
    }

    // ===== 9. ENHANCED RANDOM GENERATION =====
